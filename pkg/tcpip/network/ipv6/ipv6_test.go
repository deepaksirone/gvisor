...
// TestAddNilIpv6Address tests to make sure adding a nil IPv6 adddress does not
// cause AddAddress to panic. This test is in response to b/140943433.
func TestAddNilIpv6Address(t *testing.T) {
	s := stack.New([]string{ProtocolName}, nil, stack.Options{})
	if err := s.CreateNIC(1, &stubLinkEndpoint{}); err != nil {
		t.Fatalf("CreateNIC(_) = %s", err)
	}

	if err := s.AddAddress(1, ProtocolNumber, tcpip.Address([]byte(nil))); err != nil {
