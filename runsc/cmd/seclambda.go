...
func (g *Seclambda) Execute(_ context.Context, f *flag.FlagSet, args ...interface{}) subcommands.ExitStatus {
	...
	//sandFile := os.NewFile(uintptr(g.ioFDs[0]), "sandbox file")
	/*
		buf := make([]byte, 4)
		if r, err := sandFile.Read(buf); err != nil || r < 1 {
			log.Debugf("[Seclambda] Unable to read from sandbox file")
		}*/

	//log.Debugf("[Seclambda] Read Buffer: %v", buf)
	guard := seclambda.New(g.ctrAddr, g.ctrPort, g.ioFDs[0], g.ioFDs[1])
...
func (g *Seclambda) writeMounts(mounts []specs.Mount) error {
